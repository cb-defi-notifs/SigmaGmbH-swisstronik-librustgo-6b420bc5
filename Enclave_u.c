...
typedef struct ms_ecall_init_seed_node_t {
	sgx_status_t ms_retval;
} ms_ecall_init_seed_node_t;
...
typedef struct ms_ecall_start_seed_server_t {
	int ms_fd;
	sgx_quote_sign_type_t ms_quote_type;
} ms_ecall_start_seed_server_t;

typedef struct ms_ecall_request_seed_t {
	int ms_fd;
	sgx_quote_sign_type_t ms_quote_type;
} ms_ecall_request_seed_t;
...
static sgx_status_t SGX_CDECL Enclave_u_recvmsg_ocall(void* pms)
{
	...
	ms->ms_retval = u_recvmsg_ocall(ms->ms_error, ms->ms_sockfd, ms->ms_msg_name, ms->ms_msg_namelen, ms->ms_msg_namelen_out, ms->ms_msg_iov, ms->ms_msg_iovlen, ms->ms_msg_control, ms->ms_msg_controllen, ms->ms_msg_controllen_out, ms->ms_msg_flags, ms->ms_flags);
...
static sgx_status_t SGX_CDECL Enclave_u_sendmsg_ocall(void* pms)
{
	...
	ms->ms_retval = u_sendmsg_ocall(ms->ms_error, ms->ms_sockfd, ms->ms_msg_name, ms->ms_msg_namelen, ms->ms_msg_iov, ms->ms_msg_iovlen, ms->ms_msg_control, ms->ms_msg_controllen, ms->ms_flags);
...
{
	sgx_status_t status;
	ms_ecall_start_seed_server_t ms;
	ms.ms_fd = fd;
